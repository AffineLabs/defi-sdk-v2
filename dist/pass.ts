"use strict";
import { AffinePass__factory } from "../src/typechain";
import { EthRPC, PassEthAddress } from "../src/constants";
import { providers } from "ethers";

Object.defineProperty(exports, "__esModule", { value: true });
exports.bridgePass =
  exports.ccipFee =
  exports.getAffinePassBridge =
 ...
 * @returns
 */
async function getAffinePassBridge(destinationChainId: number, provider: any) {
  if (destinationChainId === 1) {
    return typechain_1.AffinePassBridge__factory.connect(
      constants_1.PassPolygonBridgeAddress,
      provider,
    );
  } else {
    return typechain_1.AffinePassBridge__factory.connect(
      constants_1.PassEthBridgeAddress,
      provider,
    );
  }
}
 ...
 * @returns
 */
async function ccipFee(destinationChainId: number, provider: any) {
  ...
  const _fee = await affinePass.ccipFee(
    exports.CCIP_NETWORK_SELECTOR[destinationChainId],
  );
...
async function bridgePass(
  ...
  if (bridge) {
    ...
    const gasLimit = (
      ...
      .mul(12)
      .div(10);
    const tx = await bridge.bridgePass(
      exports.CCIP_NETWORK_SELECTOR[destinationChainId],
      destinationAddress,
      tokenId,
      { value, gasLimit },
